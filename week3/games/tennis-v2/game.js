...
function move(p,x,y,speed,dt){let dx=x-p.x,dy=y-p.y,d=Math.hypot(dx,dy);if(d>.03){const scale=Math.min(1,speed*dt/d);p.x+=dx*scale;p.y+=dy*scale;p.step+=dt*12;p.travel=(p.travel||0)+d*scale;p.moveDx=dx;p.lastMoved=clock}else p.step*=.9;}
...
function draw(){if(!playerSpritesReady){ctx.fillStyle='#10222b';ctx.fillRect(0,0,W,H);ctx.fillStyle='#e1eacc';ctx.font='24px sans-serif';ctx.textAlign='center';ctx.fillText(spriteLoadError?'Player assets could not load. Reload this page.':'Loading recorded player motions…',640,360);return}ctx.drawImage(background,0,0);poly([project(-5.485,-BASE),project(5.485,-BASE),project(5.485,BASE),project(-5.485,BASE)],'#49669c','#f2f5eb',3);for(const x of [-SINGLE,SINGLE])worldLine(x,-BASE,x,BASE);worldLine(-SINGLE,-6.4,SINGLE,-6.4);worldLine(-SINGLE,6.4,SINGLE,6.4);worldLine(0,-6.4,0,6.4);worldLine(0,-BASE,0,-BASE+.4);worldLine(0,BASE,0,BASE-.4);
